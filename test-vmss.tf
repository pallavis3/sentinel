...
resource "azurerm_virtual_machine_scale_set_extension" "tlz_extension_linux" {
   ...
   commandToExecute = "powershell -NoProfile -windowstyle hidden -ExecutionPolicy Unrestricted -Command \"Expand-Archive -LiteralPath '$(path.module)/powershell-win2019.zip' -DestinationPath C:\reference \""
    managedIdentity = {}
    })
